...
typedef struct Token {
    enum {
        TOKEN_ID, TOKEN_INT, TOKEN_SEMICOLON, TOKEN_COMMA,
...
typedef struct AST {
    enum {
        ...
        AST_BLOCK, AST_FUNCCALL, AST_PROG, AST_NOOP
    } type;
    union {
...
enum Reg {RDI = 0, RSI, RDX, RCX, R8, R9};
...
// grammar
// program -> {func}
// func -> "int" "main" "(" ["int" id {"," "int" id}] ")" (block | ";")
// block -> "{" {block-item} "}"
...
// unary -> unop unary
// primary -> "(" exp ")" | int | id | func-call | EMPTY
...
void write_ret(FILE *fp, AST *ast, SymTable *env, State state);
void write_prog(FILE *fp, AST *ast);
void write_ast(FILE *fp, AST *ast, SymTable *env, State state);
// symtable funcs
VarMap *varmap_init(char *id, int offset);
void varmap_free(VarMap *var);
void funcmap_free(FuncMap *fn);
FuncMap *funcmap_init(char *id, int argc);
SymTable *symtable_init(int type, int size, SymTable *parent);
void symtable_free(SymTable *st);
void symtable_resize(SymTable *st);
void symtable_insert(SymTable *st, void *item);
bool symtable_contains(SymTable *st, char *id);
bool symtable_cur_contains(SymTable *st, char *id);
VarMap *symtable_get_varmap(SymTable *st, char *id);
VarMap *symtable_cur_get_varmap(SymTable *st, char *id);
FuncMap *symtable_get_funcmap(SymTable *st, char *id);

// global symtable (temporary)
// SymTable *global_st;
// int global_stack_index = 8;
int GLOBAL_LABEL = 0;
SymTable *GLOBAL_FUNCMAP;
...
int main(int argc, char **argv) {
    ...
    // validation
    GLOBAL_FUNCMAP = symtable_init(FUNCMAP, ST_BASE_SIZE, NULL);
    validate(ast);
...
Token *get_next_token(FILE *fp) {
    ...
    switch (c) {
        ...
        case ',': return token_init(TOKEN_COMMA, NULL);
        case '!':
            if (peek(fp) == '=') {
                getc(fp);
...
void ast_free(AST *ast) {
    switch (ast->type) {
        ...
        case AST_FUNC:
            ...
            for (int i = 0; i < ast->node.function.argc; i++) {
                free(ast->node.function.argv[i]);
            }
            free(ast->node.function.argv);
            free(ast->node.function.id);
            break;
        case AST_FUNCCALL:
            for (int i = 0; i < ast->node.func_call.argc; i++) {
                ast_free(ast->node.func_call.argv[i]);
            }
            free(ast->node.func_call.argv);
            free(ast->node.func_call.id);
            break;
        case AST_UNOP:
        ...
        case AST_WHILE:
            ...
            break;
        case AST_PROG:;
            AST **node = ast->node.program.funcs;
            while (*node != NULL) {
                ast_free(*node);
                node++;
            }
            free(ast->node.program.funcs);
            break;
        default:
    }
    free(ast);
...
AST *parse_call(Parser *parser, AST *id) {
    AST *node = ast_init(AST_FUNCCALL);
    node->node.func_call.id = strdup(id->node.id.value);
    node->node.func_call.argc = 0;
    AST **argv = dmalloc(sizeof(AST *));
    int size = 1;
    parser_eat(parser, TOKEN_LPAREN);
    Token *token = parser->cur_token;
    for (; token->type != TOKEN_RPAREN; token = parser->cur_token) {
        argv = drealloc(argv, (size + 1) * sizeof(AST *));
        argv[size++ - 1] = parse_expr(parser);
        node->node.func_call.argc++;
        Token *tmp = parser->cur_token;
        if (tmp->type == TOKEN_COMMA) parser_eat(parser, TOKEN_COMMA);
    }
    parser_eat(parser, TOKEN_RPAREN);
    node->node.func_call.argv = argv;
    return node;
}
...
AST *parse_primary(Parser *parser) {
    ...
    switch (token->type) {
        ...
        case TOKEN_ID:
            ...
            if (token->type == TOKEN_LPAREN) node = parse_call(parser, node);
            break;
        case TOKEN_LPAREN:
            parser_eat(parser, TOKEN_LPAREN);
...
AST *parse_func(Parser *parser) {
    ...
    Token *token = parser->cur_token;
    node->node.function.id = strdup(token->value);
    parser_eat(parser, TOKEN_ID);
    ...
    token = parser->cur_token;
    for (; token->type != TOKEN_RPAREN; token = parser->cur_token) {
        parser_eat(parser, TOKEN_ID); // match "int"

        Token *tmp = parser->cur_token;
        argv = drealloc(argv, (size + 1) * sizeof(char *));
        argv[size++ - 1] = strdup(tmp->value);
        parser_eat(parser, TOKEN_ID);
        node->node.function.argc++;

        tmp = parser->cur_token;
        if (tmp->type == TOKEN_COMMA) parser_eat(parser, TOKEN_COMMA);
    }
    parser_eat(parser, TOKEN_RPAREN);
    node->node.function.argv = argv;

    // parse func body
    token = parser->cur_token;
    if (token->type == TOKEN_SEMICOLON) {
        node->node.function.body = ast_init(AST_NOOP);
        parser_eat(parser, TOKEN_SEMICOLON);
    } else {
...
AST *parse_prog(Parser *parser) {
    AST *node = ast_init(AST_PROG);
    AST **funcs = dmalloc(sizeof(AST *));
    int size = 1;
    Token *token = parser->cur_token;
    while (token != NULL) {
        funcs = drealloc(funcs, (size + 1) * sizeof(AST *));
        funcs[size++ - 1] = parse_func(parser);
        token = parser->cur_token;
    }
    funcs[size-1] = NULL;
    node->node.program.funcs = funcs;
    return node;
}
...
void debug_print_token(Token *token) {
    ...
    switch (type) {
        ...
        case TOKEN_COMMA: puts("TOKEN_COMMA ','"); break;
...
void debug_print_ast(AST *ast, int n) {
    switch (ast->type) {
        ...
        case AST_FUNC:
            ...
            print_spaces(n); printf("argc: %d\n", ast->node.function.argc);
            print_spaces(n); puts("argv:");
            for (int i = 0; i < ast->node.function.argc; i++) {
                print_spaces(n + 2);
                printf("%s\n", ast->node.function.argv[i]);
            }
        ...
        case AST_FUNCCALL:
            ...
            n += 2;
            print_spaces(n); printf("id: %s\n", ast->node.func_call.id);
            print_spaces(n); printf("argc: %d\n", ast->node.func_call.argc);
            print_spaces(n); puts("argv:");
            for (int i = 0; i < ast->node.func_call.argc; i++) {
                debug_print_ast(ast->node.func_call.argv[i], n + 2);
            }
            break;
        case AST_PROG:
            print_spaces(n);
            ...
            AST **node = ast->node.program.funcs;
            while (*node != NULL) {
                debug_print_ast(*node, n + 2);
                node++;
            }
    }
}

bool is_func_definition(AST *func) {
    if (func->type == AST_FUNC) {
        AST *body = func->node.function.body;
        if (body->type == AST_BLOCK) return true;
    }
    return false;
}
...
void validate(AST *ast) {
    ...
    for (AST **i = ast->node.program.funcs; *i != NULL; i++) {
        AST *fni = *i;
        FuncMap *map = funcmap_init(fni->node.function.id, fni->node.function.argc);
        symtable_insert(GLOBAL_FUNCMAP, map);
        for (AST **j = i + 1; *j != NULL; j++) {
            AST *fnj = *j;
            if ((is_func_definition(fni) && is_func_definition(fnj)) &&
                (strcmp(fni->node.function.id, fnj->node.function.id) == 0)) {
                fprintf(stderr, "err redifinition of %s\n", fni->node.function.id);
                exit(1);
            }
            if (strcmp(fni->node.function.id, fnj->node.function.id) == 0 &&
                    fni->node.function.argc != fnj->node.function.argc) {
                fprintf(stderr,
                        "err number of arguments for %s doesn't match prototype",
                        fni->node.function.id);
                exit(1);
            }
        }
...
char *write_reg(int reg) {
    switch (reg) {
        case RDI: return strdup("rdi"); break;
        case RSI: return strdup("rsi"); break;
        case RDX: return strdup("rdx"); break;
        case RCX: return strdup("rcx"); break;
        case R8: return strdup("r8"); break;
        case R9: return strdup("r9"); break;
    }
    return NULL;
}
...
void write_id(FILE *fp, AST *ast, SymTable *env) {
    ...
    VarMap *var = symtable_get_varmap(env, ident);
    if (var == NULL) {
...
void write_funccall(FILE *fp, AST *ast, SymTable *env, State state) {
    char *ident = ast->node.func_call.id;
    FuncMap *map = symtable_get_funcmap(GLOBAL_FUNCMAP, ident);  
    if (map == NULL) {
        fprintf(fp, "err undefined function %s\n", map->id);
        exit(1);
    }
    if (ast->node.func_call.argc != map->argc) {
        fprintf(fp, "err wrong number of arguments for function %s\n", map->id);
        exit(1);
    }

    AST **args = ast->node.func_call.argv;
    for (int i = 0; i < ast->node.func_call.argc; i++) {
        write_ast(fp, args[i], env, state);
        if (i < 6) {
    ...
    fprintf(fp, "\tcall %s\n", ident);
    if (ast->node.func_call.argc > 6) {
        int bytes_to_dealloc = (ast->node.func_call.argc - 6) * 8;
...
void write_declare(FILE *fp, AST *ast, SymTable *env, State state) {
    ...
    fputs("\tpush rax\n", fp);
    VarMap *map = varmap_init(ident, *(state.stack_index));
    symtable_insert(env, map);
    *(state.stack_index) += 8;
}
...
void write_block(FILE *fp, AST *ast, SymTable *env, State state) {
    SymTable *cur_env = symtable_init(VARMAP, ST_BASE_SIZE, env);
    AST **items = ast->node.block.blk_items;
...
void write_func(FILE *fp, AST *ast) {
    if (!is_func_definition(ast)) return;
    fprintf(fp, "\t.globl %s\n", ast->node.function.id);
    // func label and epilogue
    fprintf(fp, "%s:\n", ast->node.function.id);
    fputs("\tpush rbp\n", fp);
    fputs("\tmov rbp, rsp\n", fp);

    // move params to symtable
    int stack_index = 8, prev_si = 8;
    SymTable *env = symtable_init(VARMAP, ST_BASE_SIZE, NULL);
    for (int i = 0; i < ast->node.function.argc; i++) {
        char *ident = ast->node.function.argv[i];
        VarMap *map = varmap_init(ident, stack_index);
        symtable_insert(env, map);
        if (i < 6) {
...
void write_prog(FILE *fp, AST *ast) {
    ...
    AST **funcs = ast->node.program.funcs;
    while (*funcs != NULL) {
        write_func(fp, *funcs);
        funcs++;
    }
...
void write_ast(FILE *fp, AST *ast, SymTable *env, State state) {
    switch (ast->type) {
        ...
        case AST_BREAK:
            ...
            break;
        case AST_FUNCCALL:
            write_funccall(fp, ast, env, state);
        default:
    }
}
...
FuncMap *funcmap_init(char *id, int argc) {
    FuncMap *fn = dmalloc(sizeof(FuncMap));
    fn->id = strdup(id);
    fn->argc = argc;
    return fn;
}
...
void symtable_free(SymTable *st) {
    ...
    for (int i = 0; i < st->used; i++) {
        st->type == VARMAP 
            ? varmap_free((VarMap *)st->arr[i]) 
            : funcmap_free((FuncMap *)st->arr[i]);
    }
    free(st->arr);
    free(st);
}
...
void symtable_resize(SymTable *st) {
    void **new_arr = dmalloc((st->size * 2) * sizeof(void *));
...
VarMap *symtable_get_varmap(SymTable *st, char *id) {
    SymTable *stb = st;
    for (; stb != NULL; stb = stb->parent) {
        VarMap *res = (VarMap *)symtable_cur_get_varmap(stb, id);

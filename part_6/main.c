...
// code gen funcs
void setup(FILE *fp);
void write_unop(FILE *fp, AST *ast, int *x, bool flag);
...
void write_declare(FILE *fp, AST *ast, int *x, bool flag);
...
AST *parse_func(Parser *parser) {
    ...
    AST **item_ls = dmalloc(sizeof(AST *));
    int size = 1;
    Token *token = parser->cur_token;
    while (token->type != TOKEN_RBRACE) {
        AST *item = parse_block_item(parser);
        item_ls = drealloc(item_ls, (size + 1) * sizeof(AST *));
        item_ls[size++ - 1] = item;
        token = parser->cur_token;
    }
    item_ls[size-1] = NULL;
    parser_eat(parser, TOKEN_RBRACE);
    node->node.function.body = item_ls;
...
void write_cond(FILE *fp, AST *ast, int *x, bool flag) {
    AST *cond = ast->node.cond.cond,
        *then_branch = ast->node.cond.then_branch,
        *else_branch = ast->node.cond.else_branch;

    write_ast(fp, cond, x, false);
    fputs("\tcmp rax, 0\n", fp);
    int tmp = *x;
    fprintf(fp, "\tje .L%d\n", tmp + 1);
    *x += 2;
    write_ast(fp, then_branch, x, flag);
    if (else_branch->type == AST_NOOP) {
        ...
        fprintf(fp, ".L%d:\n", tmp + 1);
        // *x += 1;
        write_ast(fp, else_branch, x, flag);
        fprintf(fp, ".L%d:\n", tmp + 2);
    }
}
...
bool last_is_ret(AST *ast) {
    ...
    if (ast->type == AST_COND) {
        AST *else_branch = ast->node.cond.else_branch;
...
void write_func(FILE *fp, AST *ast, int *x, bool flag) {
    ...
    if (*body == NULL) {
        ...
        AST *last = *(body - 1);
        if (!last_is_ret(last)) fputs("\tmov rax, 0\n", fp);
    }
    fputs("\tmov rsp, rbp\n", fp);
    fputs("\tpop rbp\n", fp);
    fputs("\tret\n", fp);
...
void write_ast(FILE *fp, AST *ast, int *x, bool flag) {
    switch (ast->type) {
        ...
        case AST_ASSIGN:
            ...
            break;
        case AST_DECLARE:
            write_declare(fp, ast, x, flag);
            break;
        case AST_COND:
            write_cond(fp, ast, x, flag);
            break;
        default:
    }
}

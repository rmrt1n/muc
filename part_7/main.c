...
// code gen funcs
void setup(FILE *fp);
void write_unop(FILE *fp, AST *ast, SymTable *env, int *si, int *x, bool flag);
void write_binop_operands(FILE *fp, AST *src, AST *dst,
                          SymTable *env, int *si, int *x, bool flag);
void write_binop(FILE *fp, AST *ast, SymTable *env, int *si, int *x, bool flag);
void write_assignment(FILE *fp, AST *ast, SymTable *env,
                      int *si, int *x, bool flag);
void write_declare(FILE *fp, AST *ast, SymTable *env, int *si, int *x, bool flag);
void write_cond(FILE *fp, AST *ast, SymTable *env, int *si, int *x, bool flag);
void write_func(FILE *fp, AST *ast, int *x, bool flag);
void write_ast(FILE *fp, AST *ast, SymTable *env, int *si, int *x, bool flag);
// symtable funcs
...
int main(int argc, char **argv) {
    ...
    int label = 0;
    int tmp = 8;
    write_ast(target, ast, NULL, &tmp, &label, true);
...
void write_unop(FILE *fp, AST *ast, SymTable *env, int *si, int *x, bool flag) {
    write_ast(fp, ast->node.unop.expr, env, si, x, flag);
...
void write_binop_operands(FILE *fp, AST *src, AST *dst,
                          SymTable *env, int *si, int *x, bool flag) {
    write_ast(fp, src, env, si, x, flag);
    fputs("\tpush rax\n", fp);
    write_ast(fp, dst, env, si, x, flag);
    fputs("\tpop rcx\n", fp);
}

void write_binop(FILE *fp, AST *ast, SymTable *env,
                 int *si, int *x, bool flag) {
    AST *left = ast->node.binop.left, *right = ast->node.binop.right;
    switch (ast->node.binop.type) {
        case ADD:
            write_binop_operands(fp, left, right, env, si, x, flag);
            fputs("\tadd rax, rcx\n", fp);
            break;
        case SUB:
            write_binop_operands(fp, right, left, env, si, x, flag);
            fputs("\tsub rax, rcx\n", fp);
            break;
        case MUL:
            write_binop_operands(fp, left, right, env, si, x, flag);
        ...
        case RSHIFT:
            ...
            break;
        case LOGIC_OR:
            write_ast(fp, left, env, si, x, false);
            if (left->type != AST_BINOP || left->node.binop.type != LOGIC_OR) {
                fputs("\tcmp rax, 0\n", fp);
                fprintf(fp, "\tjne .L%d\n", *x + 1);
            }
            write_ast(fp, right, env, si, x, false);
            fputs("\tcmp rax, 0\n", fp);
            fprintf(fp, "\tjne .L%d\n", *x + 1);
            break;
        case LOGIC_AND:
...
void write_assignment(FILE *fp, AST *ast, SymTable *env,
    ...
    if (type == AS) {
        write_ast(fp, ast->node.assign.expr, env, si, x, flag);
    } else {
        ...
        write_ast(fp, binop, env, si, x, flag);
    }
    int offset = var->stack_offset;
    fprintf(fp, "\tmov [rbp-%d], rax\n", offset);
}

void write_declare(FILE *fp, AST *ast, SymTable *env,
                   int *si, int *x, bool flag) {
    ...
    if (ast->node.declare.expr == NULL) {
        ...
        write_ast(fp, ast->node.declare.expr, env, si, x, flag);
    }
    fputs("\tpush rax\n", fp);
    symtable_insert(env, ident, *si);
    *si += 8;
}

void write_cond(FILE *fp, AST *ast, SymTable *env,
                int *si, int *x, bool flag) {
    AST *cond = ast->node.cond.cond,
        *then_branch = ast->node.cond.then_branch,
        *else_branch = ast->node.cond.else_branch;

    write_ast(fp, cond, env, si, x, false);
    fputs("\tcmp rax, 0\n", fp);
    int tmp = *x;
    fprintf(fp, "\tje .L%d\n", tmp + 1);
    *x += 2;
    write_ast(fp, then_branch, env, si, x, flag);
    if (else_branch->type == AST_NOOP) {
        ...
        fprintf(fp, ".L%d:\n", tmp + 1);
        // *x += 1;
        write_ast(fp, else_branch, env, si, x, flag);
        fprintf(fp, ".L%d:\n", tmp + 2);
    }
}
...
void write_block(FILE *fp, AST *ast, SymTable *env,
                 int *si, int *x, bool flag) {
    ...
    for (; *items != NULL; items++) {
        ...
        if (item->type == AST_DECLARE) {
            write_declare(fp, item, cur_env, si, x, flag);
        } else {
...
void write_func(FILE *fp, AST *ast, int *x, bool flag) {
    ...
    if (*items == NULL) {
        ...
        int stack_index = 8;
        write_block(fp, body, env, &stack_index, x, flag);
        for (; *items != NULL; items++);
        // AST *last = *(items - 1);
        if (!last_is_ret(*(items - 1))) {
            ...
            write_ast(fp, tmp_ret, env, &stack_index, x, flag);
        }
    }
}

void write_ast(FILE *fp, AST *ast, SymTable *env,
               int *si, int *x, bool flag) {
    switch (ast->type) {
        case AST_INT:
        ...
        case AST_ID:
            ...
            break;
        case AST_RET:
            write_ast(fp, ast->node.ret_stmt.expr, env, si, x, flag);
    fputs("\tmov rsp, rbp\n", fp);
    fputs("\tpop rbp\n", fp);
    fputs("\tret\n", fp);
            break;
        case AST_FUNC:
            write_func(fp, ast, x, flag);
            break;
        case AST_UNOP:
            write_unop(fp, ast, env, si, x, flag);
            break;
        case AST_BINOP:
            write_binop(fp, ast, env, si, x, flag);
            break;
        case AST_ASSIGN:
            write_assignment(fp, ast, env, si, x, flag);
            break;
        case AST_DECLARE:
            write_declare(fp, ast, env, si, x, flag);
            break;
        case AST_COND:
            write_cond(fp, ast, env, si, x, flag);
            break;
        case AST_BLOCK:
            write_block(fp, ast, env, si, x, flag);
            break;
        default:
    }
}
